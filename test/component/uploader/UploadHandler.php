...
/*
 * Usage: new UploadHandler($path, $options);
 * i.e.
 * function upload() {
 *      error_reporting(E_ALL | E_STRICT);
 *      new UploadHandler('files', [
 *          'accept_file_types' => '/(.*?)\.(((?!php\d?$).)*$)/i'
 *      ]);
 * }
 */
...
class UploadHandler
{
    ...
    function __construct($path = null, $options = null, $initialize = true, $error_messages = null)
    {
        $this->response = array();
        $this->options = array(
    ...
    protected function handle_uploaded_file($file_path, $raw_name, $file, $file_size, $content_range)
    {
        $hash = md5(hash_file('sha256', $file_path)) . '-' . hash_file('md5', $file_path) . '.' . pathinfo($file_path, PATHINFO_EXTENSION);
        $newName = $this->get_unique_filename($file_path, $hash, null, null, null, null, $content_range);
        $dirPath = dirname($file_path);
        rename($file_path, $dirPath . DIRECTORY_SEPARATOR . $newName);
        $CI = &get_instance();
        $CI->db->trans_start();
    ...
    protected function handle_file_upload($raw_name, $uploaded_file, $name, $size, $type, $error,
                                          $index = null, $content_range = null)
    {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                $insert_id = $this->handle_uploaded_file($file_path, $raw_name, $file, $file_size, $content_range);
                $file->insert_id = $insert_id ? $insert_id : 0;
                $file->url = str_replace('\\', '/', $this->get_download_url($file->name));
                /*
        ...
        if ($upload) {
            if (is_array($upload['tmp_name'])) {
                ...
                foreach ($upload['tmp_name'] as $index => $value) {
                    ...
                    $name = sha1($_file_name) . '.' . $extension;

                    if (isset($_POST['fid']) && isset($_POST['uuid'])) {
                        $name = sha1($_POST['uuid'] . $_POST['fid']) . '-' . $name;
                    }
                ...
                $_file_name = $file_name ? $file_name : $upload['name'];
                $extension = pathinfo($_file_name, PATHINFO_EXTENSION);
                $name = sha1($_file_name) . '.' . $extension;
